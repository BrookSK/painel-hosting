...
{
    private const TIPO    = 'equipe';
    private const TABELA  = 'users';
    private const EXPIRA  = 3600; // 1 hora

    // ── Passo 1: formulário de solicitação ──────────────────
    public function formulario(Requisicao $req): Resposta
    {
        return Resposta::html(View::renderizar(
            __DIR__ . '/../../Views/equipe/reset-senha.php',
            ['etapa' => 'solicitar', 'ok' => '', 'erro' => '']
        ));
    }

    // ── Passo 1: processar solicitação ──────────────────────
    ...
    private function enviarEmail(string $email, string $token): void
    {
        try {
            ...
            $corpo   = '<p style="margin:0 0 12px;">Você solicitou a redefinição de senha do Painel da Equipe.</p>'
